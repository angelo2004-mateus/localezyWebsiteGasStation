...
const Contact = () => {
  ...
  const email = 'angelomateusnerysouza@gmail.com';
  const encodedEmail = encodeURIComponent(email)
  const url = `https://formsubmit.co/${encodedEmail}`;

  return (
    <section id='contact' className='flex h-[50vh] w-full bg-[#EEEEEE] px-[40px]'>
        <form method='POST' action={url} className='m-auto flex flex-col items-center justify-center px-[20px] gap-y-5 w-[1200px] h-[300px] rounded-lg bg-gray-300'>
